...
class SlackBot(OutputChannel):
    def __int__(self,slack_verification_token,channel):
...
class SlackIput(HttpInputComponent):
    ...
    def blueprint(self,on_new_message):
        ...
        @slack_webhook.route('/slack/events',methods =['POS'])
        def event():
            if request.json.get('type')=='url_verification':
                return request.json.get('challege'),200
        ...
        if request.json.get('token')== self.slack_client and request.json.get('type')== 'event_callback':
            ...
            bot  =messaging_events.get('bot.id')
